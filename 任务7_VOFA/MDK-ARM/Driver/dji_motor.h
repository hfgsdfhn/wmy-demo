#ifndef DJI_MOTOR_H
#define DJI_MOTOR_H

#include <stdint.h>

#include "Bsp_Can.h"

#define DJI_MOTOR_FEEDBACK_ID_BASE  0x201U
#define DJI_MOTOR_CONTROL_ID_1_4    0x200U
#define DJI_MOTOR_CONTROL_ID_5_8    0x1FFU
#define DJI_MOTOR_MIN_ID            1U
#define DJI_MOTOR_MAX_ID            8U
#define DJI_M2006_MAX_CURRENT        10000
#define DJI_M3508_MAX_CURRENT        16384

typedef enum
{
    DJI_MOTOR_TYPE_M2006 = 0,
    DJI_MOTOR_TYPE_M3508 = 1
} dji_motor_type_t;

typedef struct
{
    uint8_t id;             //电机id，1-8
    dji_motor_type_t type;  //电机种类
    uint16_t encoder;       //当前编码器值
    uint16_t last_encoder;  //上一次编码器值
    int32_t encoder_turns;  //编码器圈数，正数表示正向转动，负数表示反向转动
    int32_t total_encoder;  //总编码器值，等于 encoder + encoder_turns * 8192
    uint8_t encoder_ready;  //编码器是否已准备好，0表示未准备好，1表示已准备好
    int16_t speed_rpm;      //电机转速，单位为RPM，正数表示正向转动，负数表示反向转动
    int16_t current;        //电机实际电流，单位为mA，正数表示正向转动，负数表示反向转动
    uint8_t temperature;    //电机温度，单位为摄氏度
    int16_t target_current; //电机目标电流，单位为mA，正数表示正向转动，负数表示反向转动
    bsp_can_t *can;
} dji_motor_t;

void DjiMotor_Init(dji_motor_t *motor, bsp_can_t *can, uint8_t id,
                   dji_motor_type_t type);
void DjiMotor_SetCurrent(dji_motor_t *motor, int16_t current);
void DjiMotor_ParseFeedback(dji_motor_t *motor, const uint8_t *data);

bool DjiMotor_SendGroup(dji_motor_t *motor, uint8_t number);

#endif
