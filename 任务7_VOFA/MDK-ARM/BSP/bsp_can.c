 ...
 */
void BspCan_SetRxCallback(bsp_can_t *can, bsp_can_rx_callback_t callback)
{
    if (can != 0)
    {
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *handle)
{
    ...
    while (HAL_CAN_GetRxFifoFillLevel(handle, CAN_RX_FIFO0) > 0U)
    {
        ...
        {
            can->rx_callback(can, header.StdId, data, header.DLC);
        }
    }
}
#endif
                               ...
                               uint32_t interrupt_flags)
{
    ...
    while (HAL_FDCAN_GetRxFifoFillLevel(handle, FDCAN_RX_FIFO0) > 0U)
    {
        ...
        if ((header.IdType == FDCAN_STANDARD_ID)
            && (can->rx_callback != 0))
        {
            can->rx_callback(can, header.Identifier, data,
