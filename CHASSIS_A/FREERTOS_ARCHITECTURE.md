...
| `controlTask` | Realtime | 2048 B | 1 ms | `app_rtos.c` |
| `sensorTask` | High1 | 1536 B | 任务标志 | `app_rtos.c` |
| `canRxTask` | High | 1536 B | CAN 队列 | `app_rtos.c` |
| `safetyTask` | High | 1024 B | 10 ms | `app_rtos.c` |
| `mechanismTask` | AboveNormal1 | 1536 B | 5 ms | `app_rtos.c` |
| `navigationTask` | AboveNormal | 3072 B | 10 ms | `app_rtos.c` |
| `communicationTask` | Normal | 2048 B | 通信队列 | `app_rtos.c` |
| `monitorTask` | Low | 1024 B | 100 ms | `app_rtos.c` |
...
| `trajectoryQueue` | 32 | `PathPoint` | FIFO，队满返回失败 |
...
- 队列消息定义在 `app_messages.h`，轨迹队列使用 `path.h` 中的 `PathPoint`；状态快照和事件定义在 `app_state.h`，业务代码不依赖 CMSIS-RTOS。
- `app_rtos.h` 只公开数据提交、状态快照、事件和故障接口；队列与任务句柄均为内部对象。
- 任务入口、队列句柄与调度辅助函数均为 `app_rtos.c` 的私有实现。
